...
const EDU = [
  ...
  { label: '普通一本/二本', weight: 105, color: '#5c6bc0' },
...
// 渲染倍速列表
const SPEEDS = [0.5, 0.75, 1, 1.25, 1.5, 2, 2.5, 3, 3.5, 4, 5, 10];
const speedList = document.getElementById('speed-list');
SPEEDS.forEach(s => {
  const pill = document.createElement('span');
  pill.className = 'speed-pill';
  pill.textContent = (s === 1 ? '1.0' : s) + 'x';
  speedList.appendChild(pill);
});
